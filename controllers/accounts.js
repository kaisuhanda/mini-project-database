...
module.exports={
    ...
    register: async (req, res, next) => {
      try {
        ...
        // Update accountId dengan ID akun yang baru dibuat
        await newAccount.update({
          referralCodeId: referralCodeRecord.id,
        });

        // Generate token JWT
         const token = jwt.sign({ accountId: newAccount.id }, process.env.SCRT_TKN);
    ...
    //             raw:true,
    //         });
    //         const {id,username,email,phone,role}= result;
    //         const token =jwt.sign({id,role}, process.env.SCRT_TKN, {
    //             expiresIn:"1h",
    //         });
    //         return res.status(200).send({
     ...
     editPassword: async (req, res, next) => {
      ...
      editProfile: async (req, res, next) => {
        try {
          ...
          // // Memeriksa apakah pengguna memiliki izin untuk mengedit profil
          // if (user.role !== "user") {
          //   return res.status(403).send({
          //     success: false,
          //     message: "Anda tidak diizinkan mengedit profil. Hanya pengguna yang dapat mengedit profil.",
          //   });
          // }
      
          // Periksa apakah pengguna mencoba mengubah data profil mereka sendiri
